...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id('VARIETE_GDD.BONO_FARMACIA') AND OBJECTPROPERTY(id, 'IsForeignKey') = 1)
...
CREATE TABLE VARIETE_GDD.USUARIO (
	USU_DNI NUMERIC(8) UNIQUE NOT NULL PRIMARY KEY,
...
CREATE TABLE VARIETE_GDD.AFILIADO ( 
	AFI_NUMERO INTEGER IDENTITY (1,1) NOT NULL PRIMARY KEY,
...
--ROUSU_USUARIO es un numeric no puede pasarlo desde Varchar, va el DNI del Usuario que es Administrador
...
--// ROL_USUARIO, PROFESIONAL Y AFILIADO
	...
	INSERT INTO [VARIETE_GDD].[ROL_USUARIO] (ROLUSU_USUARIO,ROLUSU_ROL)
	SELECT DISTINCT PRO_DNI ,'PROFESIONAL'  FROM VARIETE_GDD.PROFESIONAL
	UNION
	SELECT DISTINCT AFI_DNI,'AFILIADO' FROM VARIETE_GDD.AFILIADO
...
--// BONO CONSULTA
	...
	--				(SELECT TOP 1 CB.COM_AFILIADO FROM [VARIETE_GDD].COMPRA CB WHERE CB.COM_FECHA=M.Compra_Bono_Fecha AND CB.COM_AFILIADO=M.Paciente_Dni AND M.Paciente_Dni IS NOT NULL),
	--				M.Bono_Consulta_Numero,
	--				M.Bono_Consulta_Fecha_Impresion, 
	--				M.Plan_Med_Codigo,
	--				(SELECT TOP 1 CB.COM_CODIGO FROM [VARIETE_GDD].COMPRA CB WHERE CB.COM_FECHA=M.Compra_Bono_Fecha AND CB.COM_AFILIADO=M.Paciente_Dni AND M.Paciente_Dni IS NOT NULL)
	--FROM gd_esquema.Maestra M 
	--WHERE M.Bono_Consulta_Numero IS NOT NULL AND M.Compra_Bono_Fecha IS NOT NULL AND Paciente_Dni IS NOT NULL
